...
const SafetyChecklist: React.FC = () => {
  const safetyPoints = [
    {
      ...
      icon: <Icons.Thermometer className="h-8 w-8 text-red-500" />
    },
    {
      title: "Yaka & Breakers",
      ...
      icon: <Icons.Zap className="h-8 w-8 text-amber-500" />
    },
    {
      ...
      icon: <Icons.Plug className="h-8 w-8 text-slate-700" />
    },
    {
      title: "Water Safety",
      ...
      icon: <Icons.Droplets className="h-8 w-8 text-blue-500" />
    },
    {
      title: "Backup Health",
      ...
      icon: <Icons.BatteryCharging className="h-8 w-8 text-green-600" />
    }
  ];
  ...
  return (
    <section id="safety-checklist" className="py-20 bg-yellow-50 scroll-mt-24 border-y border-yellow-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center p-3 bg-yellow-100 rounded-full mb-4 border border-yellow-200">
            <Icons.AlertTriangle className="h-8 w-8 text-amber-600" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {safetyPoints.map((point, index) => (
            <div 
              key={index} 
              className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 border-t-4 border-amber-400 flex flex-col"
            >
          ...
          {/* Call to Action Card */}
          <div className="bg-slate-900 p-6 rounded-xl shadow-lg flex flex-col justify-center items-center text-center transform hover:scale-105 transition-transform duration-300">
            <h3 className="text-xl font-bold text-white mb-3">Found an Issue?</h3>
            <p className="text-slate-300 mb-6">Don't risk a fire or shock. Get a professional inspection today.</p>
            <a 
              href="#quote" 
              onClick={scrollToQuote}
              className="bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold py-3 px-6 rounded-lg w-full flex items-center justify-center cursor-pointer"
            >
